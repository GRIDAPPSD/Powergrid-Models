...
for ifn in glob.glob("base_taxonomy/*.glm"):
		...
		for code in model['line_configuration']:
			...
			for ii in range(0,len(rprim)):
				for jj in range(0,ii+1):
					rmat += str(rprim[ii][jj])
					xmat += str(xprim[ii][jj])
					if jj < ii:
						rmat += ' '
						xmat += ' '
				if ii < (len(rprim)-1):
	...
	if 'transformer' in model:
		...
		for xfmr in model['transformer']:
			...
			xff.write(' kva='+str(row['power_rating']))
			xff.write('\n\t~')
			if 'shunt_resistance' in row:
				nll_pct = str(1/float(row['shunt_resistance'])*100)
				xff.write(' %noloadloss='+nll_pct)
			if 'shunt_reactance' in row:
				imag_pct = str(1/float(row['shunt_reactance'])*100)
			...
			if row['connect_type'] == 'SINGLE_PHASE_CENTER_TAPPED':
				...
				xff.write('\n\t~')
				# Reactance
				xff.write(' xhl='+str(float(row['reactance'])*1.2*100))
				xff.write(' xht='+str(float(row['reactance'])*1.2*100))
				xff.write(' xlt='+str(float(row['reactance'])*0.8*100))
				# Primary winding
				xff.write('\n\t~ wdg=1')
				...
				xff.write(' %R='+str(float(row['resistance'])*0.5*100))
				# Secondary winding
				xff.write('\n\t~ wdg=2')
				xff.write(' bus='+bus2+'.1')
				xff.write(' conn=wye')
				xff.write(' kv=.120')
				kvbases.update([0.120])
				kvbases.update([0.240])
				xff.write(' %R='+str(float(row['resistance'])*100))
				# Tirtiary winding
				xff.write('\n\t~ wdg=3')
				xff.write(' bus='+bus2+'.2')
				xff.write(' conn=wye')
				xff.write(' kv=.120')
				xff.write(' %R='+str(float(row['resistance'])*100))
			else:
	...
	if 'load' in model:
		...
		for load in model['load']:
			...
			if 'constant_power_A' in row:
				...
				loadf.write(' kv='+str(float(row['nominal_voltage'])/1000))
				loadf.write(' kw='+str(PA/1000))
				loadf.write(' kvar='+str(QA/1000))
				loadf.write('\n')
			if 'constant_power_B' in row:
				...
				# assuming gld voltage is LL for delta loads and LN for wye loads
				loadf.write(' kv='+str(float(row['nominal_voltage'])/1000))
				loadf.write(' kw='+str(PB/1000))
				loadf.write(' kvar='+str(QB/1000))
				loadf.write('\n')
			if 'constant_power_C' in row:
				...
				# assuming gld voltage is LL for delta loads and LN for wye loads
				loadf.write(' kv='+str(float(row['nominal_voltage'])/1000))
				loadf.write(' kw='+str(PC/1000))
				loadf.write(' kvar='+str(QC/1000))
				loadf.write('\n')
	...
	if 'triplex_node' in model: 
		...
		for node in model['triplex_node']:
			...
			if 'power_12' in row:
				...
				loadf.write(' kw='+str(P/1000))
				loadf.write(' kvar='+str(Q/1000))
				loadf.write('\n')
			if 'power_1' in row:
				...
				if 'bustype' in model[t][o]:
					if model[t][o]['bustype'] == 'SWING':
						...
						sourcef.write(' wdg=2 kv='+\
							str(float(model[t][o]['nominal_voltage'])/1000*1.73205))
