...
# casefiles = [{'root':'R2_12_47_2',   'bases':[480.0,12470.0,100000.0]}]
...
def write_comparisons(basepath, dsspath, glmpath, rootname, voltagebases):
  ...
  if sys.platform == 'linux':
    dssname = dssname.lower()
  v1 = load_voltages (basepath + dssname + '_v.csv')
  v2 = load_voltages (dsspath + dssname + '_v.csv')
  t1 = load_taps (basepath + dssname + '_t.csv')
  t2 = load_taps (dsspath + dssname + '_t.csv')
  i1 = load_currents (basepath + dssname + '_i.csv')
  i2 = load_currents (dsspath + dssname + '_i.csv')
  s1 = load_summary (basepath + dssname + '_s.csv')
  s2 = load_summary (dsspath + dssname + '_s.csv')

  gldbus, gldv = load_glm_voltages (glmpath + glmname + '_volt.csv', voltagebases)
  gldlink, gldi = load_glm_currents (glmpath + glmname + '_curr.csv')
...
#  print (i1)
  ...
  flog = open (dsspath + dssname + '_Summary.log', 'w')
  ...
  fcsv = open (dsspath + dssname + '_Compare_Voltages_DSS.csv', 'w')
  ...
  fcsv = open (glmpath + glmname + '_Compare_Voltages_GLM.csv', 'w')
  ...
  ftxt = open (dsspath + dssname + '_Missing_Nodes_DSS.txt', 'w')
  nmissing_1 = 0
  nmissing_2 = 0
  for bus in v1:
    if not bus in v2:
  ...
  fcsv = open (dsspath + dssname + '_Compare_Currents_DSS.csv', 'w')
  ...
  fcsv = open (glmpath + glmname + '_Compare_Currents_GLM.csv', 'w')
  ...
  ftxt = open (dsspath + dssname + '_Missing_Links_DSS.txt', 'w')
  nmissing_1 = 0
  nmissing_2 = 0
  for link in i1:
    if not link in i2:
